...
int main(int argc, char* argv[]) {
	string name = "test6.jpg";
	cv::Mat image = cv::imread("C:/Users/opo/Desktop/mole" + name, cv::IMREAD_COLOR);//원본 이미지
	cv::Mat original = cv::imread("C:/Users/opo/Desktop/mole" + name, cv::IMREAD_COLOR);//원본 이미지
	cv::imshow("img", image);
	...
	while (check != 0) {
		cv::Mat black = masking(image, cloudy);
		cv::Mat rough = masking(image, cloudy - 10);
		int * index = find(black);
		int * index2 = find(rough);

		resultB = boundary(index, index2);//return result B
		...
		resultC = histogram(original);//return result C
	...
	cout << "*********************" << endl;
	cout << "색조의 다양성 정도는 : " << colorPersent << "% 입니다" << endl;
	cv::waitKey(0);
